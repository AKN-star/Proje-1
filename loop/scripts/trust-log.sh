#!/usr/bin/env bash
# BUILD 4: the trust ledger. Autonomy is per skill, not per loop - this is
# the mechanism, not just a stated intention.
...
case "${1:-}" in
  ...
  *)
    # NOTE: this `r` (pass|fail result string) is a different variable
    # from tier_of's `r` (run count) above - separate awk processes,
    # separate scope, not a bug, just don't conflate the two.
